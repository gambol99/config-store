...
func (f *KVFile) Write(data []byte, off int64) (uint32, fuse.Status) {
	...
	/*
		...
		res, err := f.Client.Get(f.Path, false, false)
		if err != nil {
			glog.Errorf("Failed to get key: %s, error: %s", f.Path, err )
			return 0, fuse.EIO
		}
		...
		_, err = f.Client.Set(f.Path, newValue.String(), uint64(0))
		if err != nil {
			glog.Errorf("Failed to write key: %s, value: %s, error: %s", f.Path, data, err )
			return 0, fuse.EIO
		}
